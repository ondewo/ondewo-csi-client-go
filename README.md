...
```shell
go get github.com/ondewo/ondewo-csi-client-go
```

Then import the package of the service you need:

```go
import csipb "github.com/ondewo/ondewo-csi-client-go/api/ondewo/csi"
```
...
> import path of release `7.1.2` is `github.com/ondewo/ondewo-csi-client-go/v7/api/ondewo/csi`. The
> `Makefile` derives the suffix from `ONDEWO_CSI_VERSION`; run `make TEST` to print the
...
import (
    ...
    csipb "github.com/ondewo/ondewo-csi-client-go/api/ondewo/csi"
)
...
func main() {
    ...
    bearerToken := os.Getenv("ONDEWO_CSI_ACCESS_TOKEN")
    ...
    // api/ondewo/csi/ for the ones this product exposes.
    client := csipb.NewExampleServiceClient(conn)

    response, err := client.ExampleMethod(ctx, &csipb.ExampleRequest{})
    if err != nil {
