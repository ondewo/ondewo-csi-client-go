...
```shell
go get github.com/ondewo/ondewo-csi-client-go/v5
```
...
> `v5.y.z` tag on a module declared without `/v5` is invisible to `go get`. This release is
> `5.5.0`, so the module declares — and every generated file imports — `github.com/ondewo/ondewo-csi-client-go/v5`.
> The `Makefile` derives the suffix from `ONDEWO_CSI_VERSION`; run `make TEST` to print the
