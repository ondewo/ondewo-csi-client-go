<div align="center">
  ...
  <h1 align="center">
    ONDEWO CSI Client Go
  </h1>
</div>

## Overview

`ondewo-csi-client-go` is the Go gRPC client of the ONDEWO CSI (Conversational Speech Interface) API. It is a
compiled version of the [ONDEWO CSI API](https://github.com/ondewo/ondewo-csi-api),
...
```shell
go get github.com/ondewo/ondewo-csi-client-go/v5@latest   ## or @v5.5.0 to pin an exact release
```

Then import the package of the service you need:

```go
import csipb "github.com/ondewo/ondewo-csi-client-go/v5/api/ondewo/csi"
```

> **The `/v5` is part of the name, not a version selector.** From major version 2 on, a Go module
...
> shorter spelling of the same module — `go get github.com/ondewo/ondewo-csi-client-go` names a
> *different*, unpublished module and fails with `invalid version: module contains a go.mod file, so
> major version must be compatible`. The suffix moves with the major version of the ONDEWO CSI API,
...
what the [GitHub releases page](https://github.com/ondewo/ondewo-csi-client-go/releases) lists and
...
```shell
git clone https://github.com/ondewo/ondewo-csi-client-go.git   ## Clone the repository
cd ondewo-csi-client-go                                        ## Change into the repo directory
...
import (
    ...
    csipb "github.com/ondewo/ondewo-csi-client-go/v5/api/ondewo/csi"
    "github.com/ondewo/ondewo-csi-client-go/v5/auth"
)
...
func main() {
    ...
    conn, err := grpc.NewClient(
        "grpc-csi.ondewo.com:443",
        grpc.WithTransportCredentials(credentials.NewTLS(&tls.Config{})),
        auth.WithBearerToken(os.Getenv("ONDEWO_CSI_ACCESS_TOKEN")),
    )
    if err != nil {
    ...
    // api/ondewo/csi/ for this product's own service, and api/ondewo/{nlu,s2t,t2s}/ for the
    // 18 vendored ones this client also exposes.
    client := csipb.NewConversationsClient(conn)

    response, err := client.ListS2SPipelines(ctx, &csipb.ListS2SPipelinesRequest{})
    if err != nil {
...
│       ├── csi                            <-----   protoc-gen-go) and *_grpc.pb.go (service
│       ├── nlu                            <-----   stubs, protoc-gen-go-grpc)
│       ├── s2t                            <----- csi is this product's own service; nlu, s2t and
│       └── t2s                            <-----   t2s are vendored by ondewo-csi-api
├── auth                                   <----- HAND WRITTEN - the `authorization: Bearer` credential
├── tests                                  <----- HAND WRITTEN - the go test suite (see Testing below)
├── ondewo-csi-api                             <----- submodule @ https://github.com/ondewo/ondewo-csi-api
...
make generate_ondewo_protos                 ## generate api/ from ondewo-csi-api/ondewo
...
* The image copies the mounted input volume into a temporary directory inside the container and
  compiles there, so the protos of `ondewo-csi-api` are never modified.
...
* `S2sStreamResponse` — scalars of three widths, a `oneof` carrying a nested message and a
  well-known `Struct` — survives `proto.Marshal` → `proto.Unmarshal` unchanged, and a
  truncated payload is rejected;
* a proto3 `optional` scalar keeps its explicit presence — an explicitly set `false` is
  transmitted and arrives as a non-nil pointer, while an unset field stays `nil`. This is the
  distinction the angular target of the same compiler once lost, which made `false`/`0`/`""`
  unsendable. `ondewo/csi/conversation.proto` declares no `optional` field of its own, so the
  assertion uses one of the S2T protos `ondewo-csi-api` vendors;
* the enum zero values are pinned with their generated name/value maps: `SipTriggerType(0)`
  is `UNSPECIFIED`, and `ControlStatus(0)` is the real state `OK` — an enum whose zero member
  is a genuine choice, so a member inserted at 0 upstream would silently re-label it;
...
* every generated `New<Service>Client` binds to a connection, and every generated **unary** stub is
  actually called over the wire and has to come back as `codes.Unimplemented` — 361 of them
  across the 19 services of this product, which proves each one marshals its request and builds a
...
* all 21 compiled `.proto` files are registered in the global descriptor registry as proto3.
...
```shell
make check_go_module_path   ## go.mod and every self-import carry the /vN that ONDEWO_CSI_VERSION implies
