...
# =====================================================================================
# ondewo-csi-client-go - Makefile
#
# The ONDEWO CSI (Conversational Speech Interface) gRPC client for go. Everything below the
# `api/` directory is GENERATED from the .proto files of the ondewo-csi-api submodule by the
...
#   make test                     # go test ./...
#
# Versioning: ONDEWO_CSI_VERSION (below) is the single source of truth and MUST
# match the ONDEWO CSI API in major and minor version.
#
...
# carry the major as a `/vN` suffix (https://go.dev/ref/mod#major-version-suffixes): a v5.1.2
# tag on a module declared without `/v5` is invisible to `go get`. Derived from the version
...
GO_MODULE_BASE_PATH=github.com/ondewo/ondewo-csi-client-go
GO_MODULE_MAJOR=$(firstword $(subst ., ,$(ONDEWO_CSI_VERSION)))
...
GO_RELEASE_TAG=v${ONDEWO_CSI_VERSION}
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO CSI Go Client ${ONDEWO_CSI_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-csi-client-go"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-csi-client-go repo
	pre-commit install
...
print_version: ## Print ONDEWO_CSI_VERSION and nothing else
	@echo "${ONDEWO_CSI_VERSION}"
...
generate_ondewo_protos: ## Generate the go stubs from the .proto files of the ondewo-csi-api submodule
	@[ -d ${ONDEWO_PROTOS_DIR} ] || { \
...
# stays invisible to `go get` once the release is tagged ("module .../ondewo-csi-client-go@v5.5.0:
...
check_go_module_path: ## Assert go.mod and every import agree with the /vN suffix ONDEWO_CSI_VERSION requires
	@declared=`sed -n 's|^module[[:space:]][[:space:]]*\([^[:space:]]*\).*|\1|p' go.mod | head -1`; \
	if [ "$$declared" != "${GO_MODULE_PATH}" ]; then \
		echo "$(RED)[ERROR]$(NC) go.mod declares module '$$declared', but ONDEWO_CSI_VERSION=${ONDEWO_CSI_VERSION} requires '${GO_MODULE_PATH}'"; \
	...
	echo "$(GREEN)[SUCCESS]$(NC) go.mod declares $$declared, matching ONDEWO_CSI_VERSION=${ONDEWO_CSI_VERSION}"
...
# only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no '## Release ONDEWO CSI Go Client ${ONDEWO_CSI_VERSION}' entry"; \
