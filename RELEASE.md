...
## Release ONDEWO CSI Go Client 5.5.0
...
* Initial release of the ONDEWO CSI (Conversational Speech Interface) gRPC client for Go. The module
  ...
  `google.golang.org/grpc` runtimes pinned by the compiler image. It exposes 19 services — its own `ondewo.csi.Conversations` plus the sixteen `ondewo.nlu.*`, `ondewo.s2t.Speech2Text` and `ondewo.t2s.Text2Speech` that `ondewo-csi-api` vendors.
...
* A real test suite under `tests/`, run by `.github/workflows/ci.yml` on go 1.25 and go 1.27. It
  ...
  are cross-checked against each other, and all 361 generated unary stubs are called for real.
...
* The module path carries the `/v5` major-version suffix its release number requires
  (`github.com/ondewo/ondewo-csi-client-go/v5`). Go resolves a module straight from its VCS path,
  and from major version 2 on that path has to carry the major
  ([module reference](https://go.dev/ref/mod#major-version-suffixes)): a `5.5.0` tag on a module
  declared without `/v5` is invisible to `go get`.
* The release targets tag each release twice on the same commit: with the ONDEWO release number
  (`5.5.0`) that the rest of the fleet uses, and with the `v`-prefixed spelling (`v5.5.0`) that is
